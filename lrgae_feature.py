...
from masks import MaskFeature, NullMask
...
def main():
    ...
    for epoch in tqdm(range(1, 1 + args.epochs), 'Node regression pretraining'):
    
        optimizer.zero_grad()
        model.train()
        remaining_graph, masked_graph = mask(data)
        loss = model.train_step_feature(remaining_graph, masked_graph)
        loss.backward()
        if args.grad_norm > 0:
            # gradient clipping
