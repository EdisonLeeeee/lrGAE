...
from lrgae.masks import AdversMask, MaskFeature, NullMask
...
parser.add_argument("--encoder_activation", default="prelu", help="Activation function for GNN encoder, (default: elu)")
parser.add_argument('--encoder_channels', type=int, default=1024, help='Channels of hidden representation. (default: 1024)')
...
parser.add_argument("--num_heads", type=int, default=8, help="Number of attention heads for GAT encoders (default: 8)")

parser.add_argument("--uniformity_dim", type=int, default=64, help="number of hidden units")
...
parser.add_argument("--alpha_0", type=float, default=1.0, help="`pow`coefficient for `sce` loss")
parser.add_argument("--alpha_T", type=float, default=2.0, help="`pow`coefficient for `sce` loss")
parser.add_argument("--gamma", type=float, default=1, help="`pow`coefficient for `sce` loss")
parser.add_argument("--lamda", type=float, default=5e-9, help="`pow`coefficient for `sce` loss")
...
parser.add_argument("--mode", default="last", help="Embedding mode `last` or `cat` (default: none)")
...
def main():
    ...
    uniformity_layer = nn.Linear(args.encoder_channels, args.uniformity_dim, bias=False)
    ...
    mask_generator = GNNEncoder(in_channels=data.num_features, 
                         ...
                         num_heads=num_heads,
                         activation=args.encoder_activation)
    advers_mask = AdversMask(mask_generator, args.encoder_channels, 2).to(device)
    ...
    for epoch in pbar:
        ...
        alpha_adv = args.alpha_0 + ((epoch / args.epochs)**args.gamma) * (args.alpha_T - args.alpha_0)
        mask_prob = advers_mask(data)
        loss, loss_mask = model.train_step(data, alpha_adv=alpha_adv, mask_prob=mask_prob, lamda=args.lamda, belta=args.belta)
